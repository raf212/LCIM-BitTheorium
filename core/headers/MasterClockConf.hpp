...
namespace AtomicCScompact
{
    ...
    public:
        ...
        bool InitMasterClockSlots(size_t max_slots, size_t allignment = 64)
        {
            ...
            packed64_t init_p = PackedCell64_t::ComposeCLK48u_64((now & MaskBits(CLK_B48)), MakeSTRL4_t(DEFAULT_INTERNAL_PRIORITY, ST_IDLE, REL_NONE, 0u, PC_MODE_CLK48));
        ...
        size_t RegisterMasterClockSlot(packed64_t given_init_clk = 0, size_t m_id = SIZE_MAX) noexcept
        {
            ...
            auto prepare = [&](packed64_t seed)->packed64_t
            {
                ...
                {
                    uint64_t now = MasterTimer48.NowTicks();
                    return PackedCell64_t::ComposeCLK48u_64((now & MaskBits(CLK_B48)),MakeSTRL4_t(DEFAULT_INTERNAL_PRIORITY,  ST_PUBLISHED, REL_NONE, 0u, PC_MODE_CLK48));
                }
                else
                {
                    return seed;
